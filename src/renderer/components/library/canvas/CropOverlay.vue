<script setup lang="ts">
import { computed, onBeforeUnmount, onMounted } from 'vue'

import { parseAspectRatio } from '@/lib/transform-math'
import { useTransformStore } from '@/stores/transform'
import type { NormalizedCropRect } from '@/types'

// ---------------------------------------------------------------------------
// Types & constants
// ---------------------------------------------------------------------------

type DragHandle = 'move' | 'n' | 's' | 'e' | 'w' | 'ne' | 'nw' | 'se' | 'sw'

const MIN_CROP_SIZE = 0.02
const GOLDEN = 0.61803398875

interface HandleDef {
  id: Exclude<DragHandle, 'move'>
  className: string
  cursor: string
}

const HANDLES: HandleDef[] = [
  { id: 'nw', className: '-left-1.5 -top-1.5', cursor: 'nwse-resize' },
  { id: 'n', className: 'left-1/2 -top-1.5 -translate-x-1/2', cursor: 'ns-resize' },
  { id: 'ne', className: '-right-1.5 -top-1.5', cursor: 'nesw-resize' },
  { id: 'e', className: '-right-1.5 top-1/2 -translate-y-1/2', cursor: 'ew-resize' },
  { id: 'se', className: '-right-1.5 -bottom-1.5', cursor: 'nwse-resize' },
  { id: 's', className: 'left-1/2 -bottom-1.5 -translate-x-1/2', cursor: 'ns-resize' },
  { id: 'sw', className: '-left-1.5 -bottom-1.5', cursor: 'nesw-resize' },
  { id: 'w', className: '-left-1.5 top-1/2 -translate-y-1/2', cursor: 'ew-resize' }
]

// ---------------------------------------------------------------------------
// Crop math
// ---------------------------------------------------------------------------

function clampRect(crop: NormalizedCropRect): NormalizedCropRect {
  const w = Math.max(MIN_CROP_SIZE, Math.min(1, crop.w))
  const h = Math.max(MIN_CROP_SIZE, Math.min(1, crop.h))
  const x = Math.min(1 - w, Math.max(0, crop.x))
  const y = Math.min(1 - h, Math.max(0, crop.y))
  return { x, y, w, h }
}

function applyMove(start: NormalizedCropRect, dx: number, dy: number): NormalizedCropRect {
  return clampRect({ x: start.x + dx, y: start.y + dy, w: start.w, h: start.h })
}

function applyResize(
  start: NormalizedCropRect,
  handle: Exclude<DragHandle, 'move'>,
  dx: number,
  dy: number,
  ratio: number | null
): NormalizedCropRect {
  const left0 = start.x
  const top0 = start.y
  const right0 = start.x + start.w
  const bottom0 = start.y + start.h

  let left = left0
  let top = top0
  let right = right0
  let bottom = bottom0

  if (handle.includes('w')) left = left0 + dx
  if (handle.includes('e')) right = right0 + dx
  if (handle.includes('n')) top = top0 + dy
  if (handle.includes('s')) bottom = bottom0 + dy

  if (ratio) {
    if (handle === 'nw' || handle === 'ne' || handle === 'sw' || handle === 'se') {
      const anchorX = handle.includes('w') ? right0 : left0
      const anchorY = handle.includes('n') ? bottom0 : top0
      const movingX = handle.includes('w') ? left : right
      const movingY = handle.includes('n') ? top : bottom

      const maxW = Math.max(MIN_CROP_SIZE, Math.abs(anchorX - movingX))
      const maxH = Math.max(MIN_CROP_SIZE, Math.abs(anchorY - movingY))

      let width = maxW
      let height = width / ratio
      if (height > maxH) {
        height = maxH
        width = height * ratio
      }

      left = handle.includes('w') ? anchorX - width : anchorX
      right = handle.includes('w') ? anchorX : anchorX + width
      top = handle.includes('n') ? anchorY - height : anchorY
      bottom = handle.includes('n') ? anchorY : anchorY + height
    }

    if (handle === 'e' || handle === 'w') {
      const width = Math.max(MIN_CROP_SIZE, Math.abs(right - left))
      const height = Math.max(MIN_CROP_SIZE, width / ratio)
      const centerY = (top0 + bottom0) / 2

      if (handle === 'w') {
        left = right0 - width
        right = right0
      } else {
        left = left0
        right = left0 + width
      }

      top = centerY - height / 2
      bottom = centerY + height / 2
    }

    if (handle === 'n' || handle === 's') {
      const height = Math.max(MIN_CROP_SIZE, Math.abs(bottom - top))
      const width = Math.max(MIN_CROP_SIZE, height * ratio)
      const centerX = (left0 + right0) / 2

      if (handle === 'n') {
        top = bottom0 - height
        bottom = bottom0
      } else {
        top = top0
        bottom = top0 + height
      }

      left = centerX - width / 2
      right = centerX + width / 2
    }
  }

  return clampRect({
    x: Math.min(left, right),
    y: Math.min(top, bottom),
    w: Math.abs(right - left),
    h: Math.abs(bottom - top)
  })
}

function guideLines(mode: 'thirds' | 'grid' | 'golden'): number[] {
  if (mode === 'grid') return [0.25, 0.5, 0.75]
  if (mode === 'golden') return [1 - GOLDEN, GOLDEN]
  return [1 / 3, 2 / 3]
}

// ---------------------------------------------------------------------------
// Store
// ---------------------------------------------------------------------------

const transformStore = useTransformStore()

// Drag state (non-reactive, used only in event handlers)
let drag: {
  handle: DragHandle
  startCrop: NormalizedCropRect
  startClientX: number
  startClientY: number
} | null = null

// ---------------------------------------------------------------------------
// Computed layout
// ---------------------------------------------------------------------------

const imageX = computed(() => transformStore.cropOverlay?.imageX ?? 0)
const imageY = computed(() => transformStore.cropOverlay?.imageY ?? 0)
const imageWidth = computed(() => transformStore.cropOverlay?.imageWidth ?? 1)
const imageHeight = computed(() => transformStore.cropOverlay?.imageHeight ?? 1)

const overlayReady = computed(
  () =>
    transformStore.cropMode &&
    !!transformStore.pendingCrop &&
    !!transformStore.cropOverlay &&
    transformStore.cropOverlay.imageWidth > 0 &&
    transformStore.cropOverlay.imageHeight > 0
)

const activeCrop = computed(
  () => transformStore.pendingCrop ?? { x: 0, y: 0, w: 1, h: 1 }
)

const cropPx = computed(() => ({
  left: imageX.value + activeCrop.value.x * imageWidth.value,
  top: imageY.value + activeCrop.value.y * imageHeight.value,
  width: activeCrop.value.w * imageWidth.value,
  height: activeCrop.value.h * imageHeight.value
}))

const lines = computed(() => guideLines(transformStore.cropGuide))

// Scrim regions (4 dark zones around the crop rect)
const scrimTop = computed(() => ({
  left: `${imageX.value}px`,
  top: `${imageY.value}px`,
  width: `${imageWidth.value}px`,
  height: `${cropPx.value.top - imageY.value}px`
}))

const scrimLeft = computed(() => ({
  left: `${imageX.value}px`,
  top: `${cropPx.value.top}px`,
  width: `${cropPx.value.left - imageX.value}px`,
  height: `${cropPx.value.height}px`
}))

const scrimRight = computed(() => ({
  left: `${cropPx.value.left + cropPx.value.width}px`,
  top: `${cropPx.value.top}px`,
  width: `${imageX.value + imageWidth.value - (cropPx.value.left + cropPx.value.width)}px`,
  height: `${cropPx.value.height}px`
}))

const scrimBottom = computed(() => ({
  left: `${imageX.value}px`,
  top: `${cropPx.value.top + cropPx.value.height}px`,
  width: `${imageWidth.value}px`,
  height: `${imageY.value + imageHeight.value - (cropPx.value.top + cropPx.value.height)}px`
}))

const cropRectStyle = computed(() => ({
  left: `${cropPx.value.left}px`,
  top: `${cropPx.value.top}px`,
  width: `${cropPx.value.width}px`,
  height: `${cropPx.value.height}px`,
  cursor: 'move'
}))

// ---------------------------------------------------------------------------
// Drag handlers
// ---------------------------------------------------------------------------

function onDragStart(event: MouseEvent, handle: DragHandle): void {
  if (!overlayReady.value) return
  event.preventDefault()
  event.stopPropagation()

  drag = {
    handle,
    startCrop: { ...activeCrop.value },
    startClientX: event.clientX,
    startClientY: event.clientY
  }
}

function onDocumentMouseMove(event: MouseEvent): void {
  if (!overlayReady.value || !drag) return

  const dx = (event.clientX - drag.startClientX) / imageWidth.value
  const dy = (event.clientY - drag.startClientY) / imageHeight.value
  const ratio = parseAspectRatio(transformStore.cropAspectRatio)

  const next =
    drag.handle === 'move'
      ? applyMove(drag.startCrop, dx, dy)
      : applyResize(drag.startCrop, drag.handle, dx, dy, ratio)

  transformStore.setPendingCrop(next)
}

function onDocumentMouseUp(): void {
  drag = null
}

onMounted(() => {
  document.addEventListener('mousemove', onDocumentMouseMove)
  document.addEventListener('mouseup', onDocumentMouseUp)
})

onBeforeUnmount(() => {
  document.removeEventListener('mousemove', onDocumentMouseMove)
  document.removeEventListener('mouseup', onDocumentMouseUp)
})
</script>

<template>
  <div v-if="overlayReady" class="pointer-events-none absolute inset-0 z-30 select-none">
    <!-- Scrim regions -->
    <div class="absolute bg-black/45" :style="scrimTop" />
    <div class="absolute bg-black/45" :style="scrimLeft" />
    <div class="absolute bg-black/45" :style="scrimRight" />
    <div class="absolute bg-black/45" :style="scrimBottom" />

    <!-- Crop rectangle -->
    <div
      class="pointer-events-auto absolute border-2 border-white/90 shadow-[0_0_0_1px_rgba(0,0,0,0.5)]"
      :style="cropRectStyle"
      @mousedown="onDragStart($event, 'move')"
    >
      <!-- Guide lines -->
      <template v-for="line in lines" :key="`guide-${line}`">
        <div
          class="absolute top-0 bottom-0 w-px bg-white/40"
          :style="{ left: `${line * 100}%` }"
        />
        <div
          class="absolute left-0 right-0 h-px bg-white/40"
          :style="{ top: `${line * 100}%` }"
        />
      </template>

      <!-- Resize handles -->
      <div
        v-for="handle in HANDLES"
        :key="handle.id"
        :class="['absolute size-3 rounded-sm border border-white/90 bg-black/60', handle.className]"
        :style="{ cursor: handle.cursor }"
        @mousedown="onDragStart($event, handle.id)"
      />
    </div>
  </div>
</template>
